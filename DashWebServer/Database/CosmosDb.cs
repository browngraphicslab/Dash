    ...
    public class CosmosDb : IDocumentRepository
    {
        ...
        public async Task<IEnumerable<T>> UpdateItemsAsync<T>(IEnumerable<T> items) where T : EntityBase
        {
            ...
            try
            {
                ...
                foreach (var item in items)
                {
                    var resourceResponse = await _client.ReplaceDocumentAsync(GetDocumentLink(item.Id), item);
                    T result = (dynamic)resourceResponse.Resource;
                    results.Add(result);
                }
        ...
        public async Task<T> UpdateItemAsync<T>(T item) where T:EntityBase
        {
            try
            {
                var resourceResponse = await _client.ReplaceDocumentAsync(GetDocumentLink(item.Id), item);
        ...
        public async Task DeleteItemAsync<T>(T item) where T : EntityBase
        {
            try
            {
                var resourceResponse = await _client.DeleteDocumentAsync(GetDocumentLink(item.Id));
            }
            catch (DocumentClientException e)
            {
                Console.WriteLine(e);
