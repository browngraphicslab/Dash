    ...
    public sealed partial class CollectionFreeformView : SelectionElement, ICollectionView
    {
        ...
        private void FreeformGrid_OnPointerReleased(object sender, PointerRoutedEventArgs e)
        {
            ...
            else if (_currReference?.IsOutput == true && _currReference?.Type == TypeInfo.List)
            {
                ...
                var sourceViewType = droppedSrcDoc.GetActiveLayout()?.GetDereferencedField<TextController>(KeyStore.CollectionViewTypeKey, null)?.Data ?? CollectionView.CollectionViewType.Schema.ToString();
                ...
                cnote.Document.GetDataDocument(null).SetField(CollectionNote.CollectedDocsKey, new DocumentReferenceController(droppedSrcDoc.GetId(), droppedField.FieldKey), true);
