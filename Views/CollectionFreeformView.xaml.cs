...
namespace Dash
{
    public sealed partial class CollectionFreeformView : UserControl
    {
        ...
        public void EndDrag(OperatorView.IOReference ioReference)
        {
            ...
            {
                CollectionView.Graph.AddEdge(ContentController.DereferenceToRootFieldModel(_currReference.ReferenceFieldModel).Id, ContentController.DereferenceToRootFieldModel(ioReference.ReferenceFieldModel).Id);
            }
            else
            {
                CollectionView.Graph.AddEdge(ContentController.DereferenceToRootFieldModel(ioReference.ReferenceFieldModel).Id, ContentController.DereferenceToRootFieldModel(_currReference.ReferenceFieldModel).Id);
            }
            if (CollectionView.Graph.IsCyclic())
            {
                if (_currReference.IsOutput)
                {
                    CollectionView.Graph.RemoveEdge(ContentController.DereferenceToRootFieldModel(_currReference.ReferenceFieldModel).Id, ContentController.DereferenceToRootFieldModel(ioReference.ReferenceFieldModel).Id);
                }
                else
                {
                    CollectionView.Graph.RemoveEdge(ContentController.DereferenceToRootFieldModel(ioReference.ReferenceFieldModel).Id, ContentController.DereferenceToRootFieldModel(_currReference.ReferenceFieldModel).Id);
                }
