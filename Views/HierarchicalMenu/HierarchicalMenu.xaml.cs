...
namespace Dash
{
    public sealed partial class HierarchicalMenu : UserControl
    {
        ...
        public void RemoveFromListSource(DocumentController controller)
        {
            ...
            {
                // not sure what happens to delegates when a prototype is removed
                    //ListItemSource.Remove(_itemsDictionary[controller]);
                    //_itemsDictionary.Remove(controller);
                // deactivate/grey out item
                _itemsDictionary[controller].Deactivate();
            }
        ...
        public void HighLightItem(DocumentController controller)
        {
            if (controller == null) return;
            this.DeselectPreviouslySelected();
            var parents = this.FindAllPrototypes(controller);
            var topLevelParent = parents[parents.Count() - 1];
            if (_itemsDictionary.ContainsKey(controller))
            {
                _itemsDictionary[controller].Select();
                _previouslySelected = _itemsDictionary[controller];
            } else if (!topLevelParent.Equals(controller) && _itemsDictionary.ContainsKey(topLevelParent))
            {
                var count = parents.Count() - 1;
                var list = _itemsDictionary[topLevelParent].List;
                ...
                list.Items[controller].Select();
                _previouslySelected = list.Items[controller];
            }
        }
        ...
        public void SelectItem(HierarchicalViewItem item)
        {
            ...
            {
                DocumentView.DocumentViews[_previouslySelected.DocController].OuterGrid.BorderBrush = new SolidColorBrush(Colors.White);
                DocumentView.DocumentViews[_previouslySelected.DocController].OuterGrid.BorderThickness = new Thickness(5);
                DocumentView.DocumentViews[_previouslySelected.DocController].SetTop();
