...
namespace Dash
{
    ...
    public class ManipulationControls : IDisposable
    {
        ...
        private static void AddConnected(List<DocumentViewModel> grouped, DocumentView docRoot, CollectionFreeformView parent)
        {
            ...
            var ordered = grouped.OrderBy(vm => (vm.DocumentController.GetField(KeyStore.PositionFieldKey) as PointController).Data.Y + vm.Height/2).ToArray();
            var length = grouped.Count;
