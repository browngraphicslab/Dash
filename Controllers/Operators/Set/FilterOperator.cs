...
namespace Dash
{
    class FilterOperator : OperatorFieldModelController
    {
        ...
        public static readonly Key FilterParameterKey = new Key("8549DC53-0185-418F-A352-3E7F1DE3E253", "Filter Parameters");
        ...
        {
            [FilterParameterKey] = TypeInfo.Document,
        ...
        {
            var parameters = (inputs[FilterParameterKey] as DocumentFieldModelController).Data;
            var docs = (inputs[InputCollection] as DocumentCollectionFieldModelController).GetDocuments();

            FilterModel.FilterType filterType;
            bool success = Enum.TryParse((parameters.GetField(FilterTypeKey) as TextFieldModelController)?.Data ?? "", out filterType);
            if (!success)
            {
                outputs[OutputCollection] = new DocumentCollectionFieldModelController();
                return;
            }
            var keyName = (parameters.GetField(KeyNameKey) as TextFieldModelController)?.Data ?? "";
            var value = (parameters.GetField(FilterValueKey) as TextFieldModelController)?.Data ?? "";
